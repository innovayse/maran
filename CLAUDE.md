...
3. Layout: proto/ (contract), backend/ (C# modular monolith), agent/ (Rust root daemon),
   frontend/ (Vue SPA), installer/, docker/ (dev only), rules/, docs/. The public site is a
   SEPARATE repository (gitlab.com/innovayse/maran-website), governed by rules/nuxt.md; this one
   git-ignores `website/`, where it is cloned when both are wanted side by side.
4. Verification commands: `maran check`, `dotnet test` (backend/),
   ...
   `maran website` (only when the site is cloned into `website/` — lint incl. content checks,
   typecheck, build, Vitest; no browser suite there by design, rules/nuxt.md).
