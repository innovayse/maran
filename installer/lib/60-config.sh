#!/usr/bin/env bash
# Step 60: generate /etc/maran/panel.env (root:<service group> 0640) with a freshly generated
...
# require_service_identity: the service account and its group must be in the environment before
# anything writes with them. The file this step writes is readable by that group and by nobody
# else, and the uid the agent admits on its socket is this account's. Both names are decided in
# install.sh and never re-spelled here.
#
# Called from the functions that use them rather than at file scope, so that SOURCING this file
# stays free of side effects — the polygon sources it to drive its two detection functions
# against a real sshd_config, and a step file that aborted on import would take that with it.
require_service_identity() {
  : "${MARAN_USER:?60-config.sh: MARAN_USER is unset; it is set by install.sh and must be in the environment}"
  : "${MARAN_GROUP:?60-config.sh: MARAN_GROUP is unset; it is set by install.sh and must be in the environment}"
}
...
write_config() {
  ...
  # Staged inside /etc/maran (root:<service group> 0750), not /tmp: a rename is only atomic within
  ...
  tmp="$(mktemp "${MARAN_CONFIG_DIR}/.panel.env.XXXXXX")"
  {
    ...
    echo "Database__Username=${MARAN_USER}"
    echo ""
...
step_config() {
  ...
  require_service_identity
  install -d -o root -g "$MARAN_GROUP" -m 0750 "$MARAN_CONFIG_DIR"
  ...
  write_agent_env
  echo "Config written with mode 0640, owner root:${MARAN_GROUP}. Secret values were not logged."
}
