...
/**
 * The create-an-SFTP-login form: the account picker, the login name, and the client-side mirror of
 * the server's validator.
 *
 * Split out of `SftpUsersPage.vue` rather than living in it, because the page was carrying three
 ...
 * nothing else (`CreateSftpUserCommandValidator`). Narrower than a legal Unix user name on
 * purpose — the value becomes a `useradd` argument, a path segment and a line in a line-oriented
 * `sshd_config` drop-in — and it excludes the underscore separator too, so account `alice` cannot
...
/** Props accepted by {@link SftpUserCreateForm}. */
...
/** Events emitted by {@link SftpUserCreateForm}. */
const emit = defineEmits<{
  /** Fired only when every client-side rule passes, carrying the request to send. */
  (e: 'submit', request: CreateSftpUserRequest): void
}>()
...
const accountError: ComputedRef<string | null> = computed(() => {
  ...
  return accountId.value.length === 0 ? t('sftp.form.errors.accountRequired') : null
})
...
const submit = (): void => {
  ...
  emit('submit', { accountId: accountId.value, name: name.value })
}

/**
 * Empties the login name and forgets that a submit was attempted, so the next login is typed into
 * a clean form rather than into one still showing the last one's value.
 *
 * The account is deliberately kept: creating several logins for one customer is the common case,
 * and re-picking the same account every time is friction with nothing behind it.
 *
 * Exposed for the page to call after the panel has accepted a create. The page cannot simply clear
 * the field itself — it lives here, and so does `submitted`, which has to be cleared with it or an
  ...
  <div class="rounded-xl border border-border-subtle bg-surface-1">
    <UiForm @submit="submit">
      <div class="grid gap-3.5 p-4.5 sm:grid-cols-2">
        ...
        <UiInput
          v-model="name"
          :label="t('sftp.form.fields.name')"
          :placeholder="t('sftp.form.placeholders.name')"
          :error="nameError"
          required
        />
      </div>
      ...
      >
        <p class="text-sm text-text-muted">{{ t('sftp.form.hint') }}</p>
        <UiButton type="submit" :disabled="submitting">{{ t('sftp.form.submit') }}</UiButton>
      </div>
    </UiForm>
  </div>
</template>
