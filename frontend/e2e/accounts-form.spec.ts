...
test('the create-account form disables browser validation and uses no native validation attributes', async ({
  ...
  const nativeAttributes = await fields.evaluateAll((inputs) =>
    inputs.flatMap((input) =>
      ['required', 'pattern', 'min', 'max', 'minlength', 'maxlength'].filter((attribute) =>
...
test('submitting the empty form shows the panel own field messages and sends no request', async ({ page }) => {
  await stubHealthy(page)
  await stubModules(page, LICENSED_ACCOUNTS)
  await stubAccounts(page, [])
...
test('submitting a name the client rules reject shows the mirrored rule message', async ({ page }) => {
  ...
  await page.getByLabel('Plan ID').fill('not-a-uuid')
  await page.getByRole('button', { name: 'Create account' }).click()
  ...
  await expect(page.getByText('Plan ID must be a valid UUID.')).toBeVisible()
})
...
test('a rejected create renders the backend RFC 7807 detail verbatim and stays on the form', async ({ page }) => {
  ...
  await page.getByLabel('Plan ID').fill('22222222-2222-2222-2222-222222222222')
  await page.getByRole('button', { name: 'Create account' }).click()
...
test('a valid submission posts the typed values and returns to the list showing the new account', async ({ page }) => {
  await stubHealthy(page)
  await stubModules(page, LICENSED_ACCOUNTS)
  await stubAccounts(page, [])
  ...
  const [request] = await Promise.all([
    page.waitForRequest(
      (candidate) => candidate.method() === 'POST' && candidate.url().includes('/api/v1/accounts'),
    ),
