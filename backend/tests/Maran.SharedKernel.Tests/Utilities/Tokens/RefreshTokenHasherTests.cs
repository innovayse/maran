...
/// <summary>The refresh token generator and the digest stored beside a session.</summary>
public sealed class RefreshTokenHasherTests
{
    ...
    /// <summary>
    /// <see cref="RefreshTokenHasher.Generate"/> now encodes with
    ...
    public void Hashing_the_same_token_twice_yields_the_same_digest()
    {
        ...
        Assert.Equal(RefreshTokenHasher.Hash(token), RefreshTokenHasher.Hash(token));
    }
}
