...
namespace Maran.Modules.Databases.Tests.Commands.ResetDatabasePassword;

/// <summary>
/// The only recovery there is for a lost database password: what it changes, who may ask for it, and
/// what it refuses to keep.
/// </summary>
public sealed class ResetDatabasePasswordCommandHandlerTests
{
    ...
    public async Task Resetting_a_password_returns_a_new_one_once_and_stores_no_copy()
    {
        ...
        var row = await read.Databases.SingleAsync(database => database.Id == world.OwnDatabaseId);
    ...
    /// <summary>Resetting another tenants database password answers not found rather than forbidden.</summary>
    [Fact]
    public async Task Resetting_another_tenants_database_password_answers_not_found_rather_than_forbidden()
    {
        // The module's most valuable IDOR. Every other cross-tenant read merely discloses that a
        // database exists; this one, if it could be pointed at somebody else's row, would hand the
        // caller a WORKING CREDENTIAL on their data. And 403 would confirm the row is real.
        var world = await WorldAsync();

        var result = await world.ResetAsync(world.StrangerDatabaseId);
    ...
    /// <summary>The agent is addressed by the user name suffix the row recorded.</summary>
    [Fact]
    public async Task The_agent_is_addressed_by_the_user_name_suffix_the_row_recorded()
    {
        // The suffix, never the fully-qualified name: the agent applies the prefix itself, so a reset
        // cannot express another tenant's login even if the row it read were wrong. And it comes
        // from the row rather than from the database's name, because the customer named the two
        // halves independently.
        var world = await WorldAsync();

        await world.ResetAsync(world.OwnDatabaseId);
    ...
    /// <summary>A reset is journalled with the database name and never with the new password.</summary>
    [Fact]
    public async Task A_reset_is_journalled_with_the_database_name_and_never_with_the_new_password()
    {
        ...
        Assert.Equal(AuditActions.DatabasePasswordReset, entry.Action);
    ...
    public async Task A_refused_reset_is_journalled_as_a_failure_naming_what_was_probed_for()
    {
        ...
        await world.ResetAsync(world.StrangerDatabaseId);
    ...
    private sealed class World
    {
        ...
        public ResetDatabasePasswordCommandHandler Handler { get; }
        ...
        /// <param name="databaseId">Which database's user to re-credential.</param>
        public async Task<Result<DatabasePasswordDto>> ResetAsync(Guid databaseId)
        {
            return await Handler.HandleAsync(
                new ResetDatabasePasswordCommand(databaseId, "203.0.113.7", "tests"),
