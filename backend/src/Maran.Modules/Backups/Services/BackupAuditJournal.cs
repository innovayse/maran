...
/// </para>
/// <para>
/// <b>The subject is the BACKUP ID, and nothing else is ever recorded.</b> Not the destination, not
/// a path, not the archive's size, not the names of the databases it holds. This journal is
/// append-only, is never deleted, and is read by the server's operator — so a line here that named
/// the archive's location would be a permanent, searchable index of where every customer's data
/// sits, written for the benefit of whoever eventually reads the journal. The id identifies the row
/// an operator is asking about, and the row holds the rest under the tenant filter.
/// </para>
/// <para>
/// The same reasoning bans the agent's own text, which can quote a path under the account's home or
/// a database name: what is recorded of a failure is its machine-stable CODE, through the row, and
/// the operator-facing sentence is logged at the agent client's boundary and stops there.
/// </para>
/// </remarks>
public sealed class BackupAuditJournal
{
    ...
    /// <param name="backupId">The backup's identifier — the subject an operator will search for.</param>
    ...
    {
        await WriteAsync(action, backupId, ipAddress, userAgent, succeeded: true, cancellationToken);
    }
    ...
    /// <param name="backupId">
    /// What it was attempted on. The backup's own identifier, which is recorded even when the caller
    /// may not see the row — so a probe for another tenant's backup still leaves a trace naming what
    /// was probed for.
    /// </param>
    /// <param name="ipAddress">The caller's address.</param>
    /// <param name="userAgent">The caller's user agent.</param>
    ...
    public async Task RecordFailureAsync(
        ...
        CancellationToken cancellationToken)
    {
        await WriteAsync(action, backupId, ipAddress, userAgent, succeeded: false, cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        await _auditWriter.WriteAsync(
            SystemAuditEntry.Create(ModuleName, action, backupId.ToString(), succeeded),
            cancellationToken);
    }

    /// <summary>Builds and stores one entry.</summary>
    /// <param name="action">The action name.</param>
    /// <param name="backupId">The backup the action was attempted on.</param>
    /// <param name="ipAddress">The caller's address.</param>
    /// <param name="userAgent">The caller's user agent.</param>
    /// <param name="succeeded">Whether the operation took effect.</param>
    /// <param name="cancellationToken">Cancellation token for the write.</param>
