...
/// construct <see cref="BackupsDbContext"/> without booting the Host. Never used at runtime — the
...
/// </remarks>
public sealed class DesignTimeDbContextFactory : IDesignTimeDbContextFactory<BackupsDbContext>
{
    ...
    public BackupsDbContext CreateDbContext(string[] args)
    {
        ...
        var optionsBuilder = new DbContextOptionsBuilder<BackupsDbContext>();
        optionsBuilder.UseNpgsql(BuildConnectionString(configuration));
        return new BackupsDbContext(optionsBuilder.Options, new DesignTimeCurrentUser());
    }
