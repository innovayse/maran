...
/// construct <see cref="FtpDbContext"/> without booting the Host. Never used at runtime — the
...
/// </remarks>
public sealed class DesignTimeDbContextFactory : IDesignTimeDbContextFactory<FtpDbContext>
{
    ...
    public FtpDbContext CreateDbContext(string[] args)
    {
        ...
        var optionsBuilder = new DbContextOptionsBuilder<FtpDbContext>();
        optionsBuilder.UseNpgsql(BuildConnectionString(configuration));
        return new FtpDbContext(optionsBuilder.Options, new DesignTimeCurrentUser());
    }
