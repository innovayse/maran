...
/// construct <see cref="IdentityDbContext"/> without booting the Host. Never used at runtime — the
...
///
/// The context needs an <see cref="IEncryptionService"/> for its encrypted TOTP column. Migration
/// generation only asks the converter for its type mapping, never for a ciphertext, so a
/// design-time stub is supplied rather than reading the panel's real encryption key into the
/// tooling — a key that never leaves the server cannot leak through a developer's shell history.
/// </remarks>
public sealed class DesignTimeDbContextFactory : IDesignTimeDbContextFactory<IdentityDbContext>
{
    ...
    public IdentityDbContext CreateDbContext(string[] args)
    {
        ...
        return new IdentityDbContext(optionsBuilder.Options, new DesignTimeEncryptionService());
    }
