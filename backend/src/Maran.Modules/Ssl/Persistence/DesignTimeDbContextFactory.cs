...
/// construct <see cref="SslDbContext"/> without booting the Host. Never used at runtime — the
...
/// </remarks>
public sealed class DesignTimeDbContextFactory : IDesignTimeDbContextFactory<SslDbContext>
{
    ...
    public SslDbContext CreateDbContext(string[] args)
    {
        ...
        var optionsBuilder = new DbContextOptionsBuilder<SslDbContext>();
        optionsBuilder.UseNpgsql(BuildConnectionString(configuration));
        return new SslDbContext(optionsBuilder.Options, new DesignTimeCurrentUser(), new DesignTimeEncryptionService());
    }
