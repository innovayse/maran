...
polygon_ensure_image() {
  ...
  if docker image inspect "$image" >/dev/null 2>&1; then
    ...
    if ! docker build -f "$context/docker/polygon/$family.Dockerfile" -t "$image" "$context" \
      >"$log" 2>&1; then
...
polygon_reconcile_counts() {
  ...
  shift 2
  local suite key expected executed row problems="" total_expected=0 total_executed=0

  for suite in "$@"; do
    key="$suite [tests/$suite.rs]"
    expected="$(awk -F'\t' -v k="$key" '$1 == k {print $4; found = 1} END {if (!found) print "?"}' \
      "$baseline" 2>/dev/null)"
    row="$(awk -F'\t' -v k="$key" '$1 == k {print $2 + $3}' "$observed" 2>/dev/null)"
    executed="${row:-0}"
  ...
  This lane will not score a suite whose scope is unknown. Record it and commit the diff:
    ...
    if [ "$executed" -lt "$expected" ]; then
      problems="$problems
  suite '$suite' executed $executed tests, but the baseline declares $expected for it
  ...
  baselined, and every test missing from it was scored against nothing."
      ...
      problems="$problems
  suite '$suite' executed $executed tests against a baseline of $expected ($((executed - expected))
  more). The suite grew without the baseline: re-record with 'maran test rust --accept'."
    fi
  done
...
$counts"
  ...
  echo "$executed tests executed across $# suites — $counts"
  return 0
}
