...
- **Banned APIs** (BannedApiAnalyzers): reading the ambient clock and spawning processes are build errors everywhere (`backend/BannedSymbols.txt`); bypassing a tenant query filter is a build error in production code only (`backend/src/BannedSymbols.txt`, which tests deliberately do not inherit — a test bypasses the filter to prove it hides the row). Every sanctioned exception is an inline suppression carrying its reason: `SystemClock`, and 24 deliberate `IgnoreQueryFilters` calls in unattended work that has no principal — `grep -rn "\.IgnoreQueryFilters()" backend/src --include=*.cs | wc -l`, which is what this sentence is FOR: a reviewer reads it to decide whether the list of bypasses has grown, so the count carries the command that regenerates it and a stale number can be caught in one line instead of believed. It has been stale twice: at twelve while the tree held 23, and at 23 while the tree held 24 (re-measured 2026-09-12). Both arrivals since the last commit are visible the same way — `git show HEAD:<file> | grep -c` against the working file — and they are `Maran.Modules/Ftp/IntegrationEvents/Handlers/AccountDeletingHandler.cs` and `Maran.Modules/Backups/Services/StartupBackupReconciler.cs`, both unattended work with no principal, which is the growth this sentence exists to surface rather than a bypass to query.
