...
- **`maran structure`** enforces what no compiler can express: one type per file, file name equals type name, `*Extensions` in `Extensions/`, interfaces in `Interfaces/`, no cross-module imports, no junk-drawer file names, and member order — a type's shape before its behaviour, in the backend and in the SPA alike. It runs first in the backend CI job.
