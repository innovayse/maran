...
Generated by `maran licenses` from metadata on disk — cargo's resolution of
`Cargo.lock`, the `.nuspec` of each restored NuGet package, and the manifest of each
installed npm module. Do not edit it by hand; run the command.
