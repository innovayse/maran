//! Everything `CreateFtpsUser` needs, already validated.
...
/// - [`FtpsUserName`] can only be built by `for_account`, which applies the
...
///   account the panel authorised. It is a different type from the SFTP login's
///   name on purpose: the two carry different group memberships, and the
