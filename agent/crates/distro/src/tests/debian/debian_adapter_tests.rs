//! Tests for the Debian-family adapter's database and SFTP facts.
//!
...
/// record of what was verified on a real Debian host, so moving a tool to a
...
/// The Debian family names each account tool at the path its packages install it at.
#[test]
fn the_debian_family_names_every_account_tool_at_its_installed_path() {
